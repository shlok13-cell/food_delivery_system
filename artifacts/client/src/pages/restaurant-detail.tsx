...
export default function RestaurantDetail() {
  ...
  const { addItem, setQty, getItemQty, totalItems, totalPrice, restaurantId: cartRestaurantId, clearCart } = useCart();
  ...
  const displayItems = vegOnly ? menuItems.filter((m) => m.is_veg) : menuItems;
  const categories = [...new Set(displayItems.map((m) => m.category))];

  useEffect(() => {
    if (categories[0] && !activeCategory) setActiveCategory(categories[0]);
  });
  ...
  function doAddToCart(item: MenuItem) {
            ...
            >
              <div className="flex items-center gap-3">
                <motion.span
                  ...
                  initial={{ scale: 1.4 }}
                  animate={{ scale: 1 }}
                  className="bg-white/20 rounded-lg w-7 h-7 flex items-center justify-center text-xs font-bold"
                >
                  {totalItems}
                </motion.span>
